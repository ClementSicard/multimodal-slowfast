...
from .build import VISION_MODEL_REGISTRY
...
@VISION_MODEL_REGISTRY.register()
...
class SlowFastGRU(nn.Module):
    ...
    def forward(
        ...
        logger.debug(
            f"S1: {reshaped_x.shape if isinstance(reshaped_x, torch.Tensor) else [i.shape for i in reshaped_x]}"
        )
