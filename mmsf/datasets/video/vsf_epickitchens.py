...
from mmsf.datasets.video.epickitchens_record import EpicKitchensVideoRecord
import mmsf.datasets.video.augmentations.transforms as transforms
import mmsf.datasets.video.utils as utils
from mmsf.datasets.video.frame_loader import pack_frames_to_video_clip
...
class EpicKitchens(torch.utils.data.Dataset):
    ...
    def spatial_sampling(
        ...
        if spatial_idx == -1:
            frames, _ = transforms.random_short_side_scale_jitter(frames, min_scale, max_scale)
            frames, _ = transforms.random_crop(frames, crop_size)
            frames, _ = transforms.horizontal_flip(0.5, frames)
        else:
            # The testing is deterministic and no jitter should be performed.
            # min_scale, max_scale, and crop_size are expect to be the same.
            assert len({min_scale, max_scale, crop_size}) == 1
            frames, _ = transforms.random_short_side_scale_jitter(frames, min_scale, max_scale)
            frames, _ = transforms.uniform_crop(frames, crop_size, spatial_idx)
        return frames
